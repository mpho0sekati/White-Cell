...
"""
    ...
    def handle_command(self, command: str, args: list) -> bool:
        ...
        if command == "exit":
            ...
            if not self._check_permission("scan.network", "trace"):
                return True
            self.handle_trace_command(args)
        elif command == "clear":
            if self.state.command_mode:
    ...
    def handle_respond_command(self, args: list) -> None:
        ...
        approval_required = action in get_approval_required_actions()
        if approval_required and self.role not in ["admin", "analyst"]:
            console.print(f"[red]Action '{action}' requires authorization.[/red]")
            governance.audit_event("soc", "respond.execute", self.role, "unauthorized", {"action": action, "target": target})
            return

        # In a real system, these would interface with actual response systems
        response_actions = {
            "contain": f"Network containment initiated for {target}.",
            "isolate": f"Host isolation in progress for {target}.",
            "reboot": f"Remote reboot initiated for {target}.",
            "patch": f"Patch deployment queued for {target}.",
            "block_ip": f"IP blocking activated for {target}.",
            "block_domain": f"Domain blocking activated for {target}.",
            "reset_password": f"Password reset initiated for {target}.",
            "quarantine_email": f"Email quarantine activated for {target}.",
        }

        if action in response_actions:
            if approval_required:
                approval_id = governance.request_approval("soc", action, self.role, {"target": target})
                console.print(f"[yellow]Approval requested for '{action}' on '{target}'. ID: {approval_id}[/yellow]")
            else:
                result = response_actions[action]
                governance.audit_event("soc", "respond.execute", self.role, "completed", {"action": action, "target": target})
                console.print(f"[green]✓ Response action completed:[/green] {result}")
        else:
            console.print(f"[red]Unsupported response action: {action}[/red]")
            governance.audit_event("soc", "respond.execute", self.role, "invalid-action", {"action": action, "target": target})
    ...
    def handle_trace_command(self, args: list) -> None:
        """Trace command to pass an IP to the Go scanner and display results in a formatted table."""
        if not args:
            console.print("[yellow]Usage: trace <ip_address>[/yellow]")
            return
        ...
        # Split IP to validate each octet
        ...
        console.print(f"[cyan]Tracing route to {ip_address}...[/cyan]")
        
        try:
            import subprocess
            import json
            
            # Look for scanner executable in the project root
            scanner_path = Path(__file__).parent.parent / "scanner.exe"
            
            if not scanner_path.exists():
                console.print(f"[red]Scanner executable not found: {scanner_path}[/red]")
                return
            
            # Run the scanner with the IP
            result = subprocess.run(
                [str(scanner_path), ip_address],
                capture_output=True,
                text=True,
            ...
            if result.returncode != 0:
                console.print(f"[red]Scanner error: {result.stderr}[/red]")
                return
            
            # Parse the JSON output from scanner
            try:
                scan_results = json.loads(result.stdout)
                
                # Create a table to display the results
                table = Table(title=f"Scan Results for {ip_address}", show_header=True, header_style="bold magenta")
                ...
                for port in open_ports:
                    service = banners.get(str(port), "Unknown Service")
                    table.add_row(str(port), service, "[bold green]OPEN[/bold green]")
                
                if not open_ports:
                    table.add_row("-", "No open ports found", "[bold red]CLOSED[/bold red]")
                
                console.print(table)
                
            except json.JSONDecodeError:
                console.print(f"[red]Could not parse scanner output as JSON: {result.stdout}[/red]")
                
        except subprocess.TimeoutExpired:
            console.print("[red]Scanner timed out after 30 seconds[/red]")
        except Exception as e:
            console.print(f"[red]Error running scanner: {str(e)}[/red]")
    ...
    def _handle_website_scan(self, args: list) -> None:
        ...
        # Check if URL is in allowlist if active scanning is requested
        if active_scan:
            allowlist = get_scan_allowlist()
        ...
        try:
            with console.status(f"[cyan]Scanning {url}{' (active)' if active_scan else ' (passive)'}...[/cyan]", spinner="dots"):
                # Call the website scanner
                results = website_scanner.scan_website(url, active=active_scan)
            
            # Display results
            if results:
                console.print(f"[green]✓ Scan completed for {url}[/green]")
                ...
                for test_name, result in results.items():
                    ...
                    result_table.add_row(test_name, status, details)
    ...
    def _add_to_allowlist(self, domain: str) -> None:
        ...
        if set_scan_allowlist(domain, add=True):
            console.print(f"[green]✓ Added {domain} to scan allowlist[/green]")
        else:
            console.print(f"[red]✗ Failed to add {domain} to scan allowlist[/red]")
    
    def _remove_from_allowlist(self, domain: str) -> None:
        """Remove a domain from the scan allowlist."""
        if set_scan_allowlist(domain, add=False):
            console.print(f"[green]✓ Removed {domain} from scan allowlist[/green]")
        else:
            console.print(f"[red]✗ Failed to remove {domain} from scan allowlist[/red]")

    def handle_trace_command(self, args: list) -> None:
        """Network trace: identify attack source and generate attribution report."""
        # Import here to avoid issues if scapy isn't installed
        try:
            from whitecell.attribution import run_attribution_scan, get_suspicious_activity_report, SCAPY_AVAILABLE
        ...
        # Check if scapy is actually available
        ...
        i = 0
        while i < len(args):
            if args[i] == "--interface" and i + 1 < len(args):
                interface = args[i + 1]
                i += 2
        ...
        try:
            ...
            governance.audit_event(
                ...
                self.role,
                "failed",
                {"error": "permission_denied", "duration": duration}
            )
        except Exception as e:
